...
#          self.position = pos
        ...
        def move_after(reference)
          ...
          if nxt.blank?
            ...
            if (nxt.get_position - reference.get_position) < 2
              self.class.connection.execute("update #{self.class.table_name} set position = position + #{self.class.list_spacing} where position >= #{nxt.get_position}")
              nxt.set_position(nxt.get_position + self.class.list_spacing)
            end
            self.set_position( (nxt.get_position + reference.get_position) / 2)
          end
        ...
        def move_before(reference)
          ...
          if prev.blank?
            ...
            self.set_position( (reference.get_position + prev.get_position) / 2)
          end
      ...
      def list_commit
        self.class.connection.execute("update #{self.class.table_name} set position = #{self.get_position} where id = #{self.id}") unless self.new_record?
